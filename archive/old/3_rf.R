...
data_split_name = args[1]
pheno <- data_phenotypes[[data_split_name]]
genos <- logreg_genos
gene_group = "gene"
...
model_data_wide <- model_data[[data_split_name]] %>%
  clean_names() %>%
...
temp = model_data[[data_split_name]]
...
exclude_ab <- temp %>% select(contains("p_")) %>% 
  apply(2, table) %>% as_tibble() %>%
  t() %>% as.data.frame() %>%
  rownames_to_column("mic_id") %>%
  as_tibble() %>% 
  rename(one = "V2", zero = "V1") %>%
  mutate(ratio = one/zero) %>%
  filter(ratio < 0.05) %>%
  pull(mic_id)
antibiotics <- p_antibiotics[!(p_antibiotics %in% c(ubi_genes, exclude_ab))]
...
for(i in 1:length(antibiotics)){
  ...
  data_rec <- recipes::recipe(x = train_data) %>%
      ...
      step_downsample(antibiotics[i], under_ratio = 1.5)
  # "prep" recipe
...
# Save model outputs
saveRDS(tune_list, paste0("outputs/",data_split_name, "_tune_list.RDS", sep = ""))
saveRDS(res_list, paste0("outputs/",data_split_name, "_res_list.RDS", sep = ""))
quit()
