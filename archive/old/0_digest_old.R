...
# manually check phenotypes to make sure they make sense against spotcheck_phenotypes_new
spotcheck_phenotypes <- filter(phenotypes, !is.na(phenotype)) %>%
  filter(!is.na(host_animal_species)) %>%
  select(host_animal_species, mic_id, breakpoint_id, animal_infection_site_uti,  contains("threshold"),  num, equal, phenotype) %>%
  unique()
spotcheck_phenotypes_new <- read_csv("data/spotcheck_phenotypes_new.csv") %>% unique() %>% filter(!is.na(host_animal_species))

a <- spotcheck_phenotypes %>% arrange(host_animal_species, mic_id, num, equal)
b <- spotcheck_phenotypes_new %>% arrange(host_animal_species, mic_id, num, equal)
...
identifiers_lookup_post <- read.csv("data/reference/identifiers_lookup_post_new.csv", encoding = "UTF-8") %>%
...
gene_metadata <- identifiers_lookup_post %>%
  ...
  unique() %>%
  select(gene_identifier, gene, gene_type, resistance_class,  gene_name_family, gene_name_family_desc, card_id, cardab_id) %>%
