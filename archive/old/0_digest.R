...
# separate mic
temp_phenotypes <- temp_phenotype_quantiles %>%
  rowwise() %>%
...
no_phenotypes <- temp_phenotypes %>% filter(is.na(phenotype))
...
review <- temp_phenotypes %>%
  ...
  group_by(num, equal, phenotype, test_result_threshold_mic_sensitive_max, test_result_threshold_mic_intermediate_min, test_result_threshold_mic_intermediate_max, test_result_threshold_mic_resistant_min) %>%
  summarize(n = n())
write.csv(review, "outputs/review_phenotypes.csv")


mic_values <- temp_phenotype_quantiles %>%
...
phenotypes <- temp_phenotypes %>%
  ...
  mutate(phenotype_iq = ifelse(num <= IQ & grepl("<|=", equal), "S", "R")) %>%
  mutate(phenotype_iq = ifelse(num < IQ & grepl(">", equal), "NI", phenotype_iq)) %>%
  filter(!is.na(value)) 
...
phenotypes <- temp_phenotypes %>%
  left_join(phenotype_quantiles) %>%
  rowwise() %>%
  mutate(phenotype = ifelse((is.na(phenotype_iq) & (num == test_result_threshold_mic_resistant_min) & (equal == ">" | equal == ">=" | equal == "=")), "R", phenotype)) %>%
  mutate(phenotype = ifelse((is.na(phenotype) & (num > test_result_threshold_mic_resistant_min) & (equal != "<=")), "R", phenotype)) %>%
  mutate(phenotype = ifelse((is.na(phenotype) & ((num == test_result_threshold_mic_intermediate_min) | (num == test_result_threshold_mic_intermediate_max)) & (equal == "=")), "I", phenotype)) %>%
  mutate(phenotype = ifelse((is.na(phenotype) & (num <= test_result_threshold_mic_sensitive_max) & (equal == "<" | equal == "<=" | equal == "=")), "S", phenotype)) %>%
  mutate(phenotype = ifelse((is.na(phenotype) & (num <= test_result_threshold_mic_intermediate_min | num <= test_result_threshold_mic_resistant_min) & (num > test_result_threshold_mic_sensitive_max) & (equal == "<=")), "NI", phenotype)) %>%
...
filter(temp_phenotypes, phenotype == "NI") %>% nrow()
filter(phenotypes, phenotype_iq == "NI" ) %>% nrow()

table(phenotypes$phenotype)
table(phenotypes$phenotype_iq)
...
sample_phenotypes <- phenotypes %>%
  ...
  mutate(iq = ifelse(!is.na(clsi), NA, iq)) %>%
...
d <- c(a,b,c)
table(d) %>% stack() %>% arrange(values) %>% head()
# breakpoint values
...
reference_data <- list(breakpoints_clean, classes, gene_metadata, genotypes, max_qt)
names(reference_data) <- c("breakpoints", "classes", "gene_metadata", "genotypes", "quantile")
