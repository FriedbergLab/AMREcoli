...
# Subset dataset and relabel all Intermediate samples as Resistant
...
# list of clsi and wt/non-wt phenotypes
data_phenotypes <- list(clsi_phenotypes, iq_phenotypes)
names(data_phenotypes) <- c("ItoR","iq75")


# Test for difference of proportions
temp_list <- data_phenotypes[[1]] %>%  
  left_join(select(sample_metadata, c("sample_id", "host_animal_common")), by = "sample_id") %>%
  group_by(mic_id, phenotype, host_animal_common) %>%
  summarize(n = n()) %>%
  group_by(mic_id) %>%
  filter(length(unique(host_animal_common)) > 1) %>%
  group_split() %>%
  lapply(function(.x){
    name <- unique(.x$mic_id)
    temp <- .x %>% 
      ungroup() %>%
      select(-mic_id) %>%
    ...
    cases = temp$`1`
    total = temp$`0` + cases 
    names(cases) <- names(total) <- temp$host_animal_common
    stat <- prop.test(cases, total)
    prop <-  stat$estimate
    names(prop) <- temp$host_animal_common 

    res <- tibble(name = name, pval = stat$p.value, data = list(stack(prop)))
    return(res)
      }) %>%
  bind_rows() %>%
  mutate(pval.adj = p.adjust(pval, method = "BH")) %>%
  mutate(sig = ifelse(pval.adj <= 0.05, TRUE, FALSE))

test <- data_phenotypes[[1]] %>%
  pivot_wider(names_from = "mic_id", values_from = "phenotype") %>%
...
# Functions to take data and reformat it into wide g
...
custom_nest <- function(pheno, genos){
  res <- genos %>%
    ...
    mutate(values = as.factor(values), phenotype = as.factor(phenotype)) %>%
...
# Functiont to filter data to testable groups 
...
usda_filter <- function(.x, gene_resistance,gene_target,  filter = TRUE){ 
  res <- .x %>%
    rename(gene_group = gene_target) %>%
    ungroup() %>%
...
# prepared data for single gene model.
usda_singlegene_data <- lapply(filtered_singlegene_data, function(.x){usda_filter(.x, gene_target = "gene", gene_resistance = gene_metadata)})
...
drug_classes <- read.csv("data/reference/drug_classes_new.csv", encoding = "UTF-8") %>%
  rename("resistance_class" = "class", "resistance_drug" = "antibiotic") %>%
...
levels_nest <- function(pheno, genos){
  res <- genos %>%
    ...
    nest() %>%
    mutate(data = map(data, function(.x){.x %>% mutate(values =factor(values, ordered = TRUE), phenotype = as.factor(phenotype)) })) %>%
...
usda_geneclass_data <- lapply(filtered_geneclass_data , function(.x){usda_filter(.x, gene_resistance = drug_classes)})
#
...
usda_binclass_data  <- lapply(filtered_binclass_data , function(.x){usda_filter(.x, gene_resistance = drug_classes)})
...
# tryCatch function for logistic regression model with animal factor
try_normal <- function(.x){tryCatch({
        return(glm( # fit model to data
          phenotype~values, # model formula
          data = .x,  
          family = "binomial"))},
          warning = function(cond){return(as.character(cond))},
          error = function(cond){return(as.character(cond))}
...
usda_singlegene_results_nore <- lapply(usda_singlegene_data, function(.x){custom_test(.x, try_normal)})
usda_geneclass_results_nore <- lapply(usda_geneclass_data, function(.x){custom_test(.x, try_normal)})
usda_binclass_results_nore <- lapply(usda_binclass_data, function(.x){custom_test(.x, try_normal)})
results <- list(usda_singlegene_results_nore, usda_geneclass_results_nore, usda_binclass_results_nore)
names(results) <- c("usda_singlegene_results_nore", "usda_geneclass_results_nore", "usda_binclass_results_nore")
saveRDS(results, "logreg_results_nore.RDS")
