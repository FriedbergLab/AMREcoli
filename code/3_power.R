...
model_data <- readRDS("outputs/ss_models.RDS") %>%  dplyr::select(mic_id, breakpoint, training, en)
...
write_csv(power_vals_grad, "outputs/power_vals.csv")
...
power_plot_antibiotic <- power_plot$data %>%
  ...
  facet_wrap(~combo) + 
  theme_classic()
ggsave("outputs/power_antibiotic.tiff", power_plot_antibiotic, height = 7, width = 12, dpi = 300, compression = "lzw")
